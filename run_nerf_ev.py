...
from load_EV import load_EV_data
...
def train():
    ...
    if args.dataset_type == 'ev_data':
        images, poses, render_poses, intrinsics, i_split, bounding_box = load_EV_data(args.datadir, args.half_res, args.testskip)
        print(f'Loaded EV images: {len(images)}, render poses: {render_poses.shape}, data_dir: {args.datadir}')
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            imageio.mimwrite(os.path.join(testsavedir, 'dist.mp4'), to8b(disps / np.max(disps)), fps=30, quality=8)
    ...
    for i in trange(start, N_iters):
        ...
        #intrinsics of the image
        K = intrinsics[img_i]
        H, W, _ = np.shape(target)

        #Get all rays from current pose            
        rays_o, rays_d = get_rays(H, W, K, torch.Tensor(pose))  # (H, W, 3), (H, W, 3)
        
        #XY coordinates of each ray
        coords = torch.stack(torch.meshgrid(torch.linspace(0, H-1, H), torch.linspace(0, W-1, W)), -1)  # (H, W, 2)
        coords = torch.reshape(coords, [-1,2])  # (H * W, 2)
        
        #select N_rand (usually 1024) different rays for training
        ...
        if i%args.i_testset==0 and i > 0:
            if False:
                ...
                gt_imgs_list = [images[i] for i in i_test]
                hwlist = [images[i].shape[0:2] for i in i_test]
                Klist = intrinsics[i_test]
                with torch.no_grad():
                    render_path(torch.Tensor(poses[i_test]).to(device), hwlist, Klist, args.chunk, render_kwargs_test, gt_imgs=gt_imgs_list, savedir=testsavedir)
